...
long f(long a, long b, long c, long d, long e, long f) {
  ...
  llvm::legacy::FunctionPassManager *FPM = new llvm::legacy::FunctionPassManager(M.get());
  std::cout << "created pass maneger\n";
  // Set up the optimizer pipeline.  Start with registering info about how the
  // target lays out data structures.
  //FPM->add(new DataLayout(EE->getDataLayout()));
  //adding loop unroll pass
  //FPM->add(llvm::createIndVarSimplifyPass());
  FPM->add(llvm::createLoopSimplifyPass());
  FPM->add(llvm::createLoopRotatePass());
  FPM->add(llvm::createLoopUnrollPass(32767));
  std::cout << "added pass\n";
  // For each function in the module
  Module::iterator it;
  //Module::iterator end = M.get()->end();
  /*for (it = M.get()->begin(); it != end; ++it) {
